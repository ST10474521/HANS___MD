...
function isSelfJid(conn, jid) {
  if (!jid) return false;
  const target = normalizeJidBase(jid);
  const selfIds = [conn.user?.id, conn.user?.lid].map(normalizeJidBase).filter(Boolean);
  return selfIds.includes(target);
}
...
function extractQuoted(mek, conn) {
  ...
  const quotedFromMe = ctx.participant
    ? isSelfJid(conn, ctx.participant)
    : isSelfJid(conn, ctx.remoteJid) || !!mek?.key?.fromMe;
